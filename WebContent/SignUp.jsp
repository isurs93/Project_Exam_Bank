  ...
  <body cellpadding="0" cellspacing="0" marginleft="0" margintop="0" width="100%" height="100%">
    ...
    <div class="card align-middle" style="width:700px; border-radius:20px;">
      ...
      <form class="form-signin" method="POST" onSubmit="return pwCheck();">
        ...
        <input type="email" id="uid" class="form-control" placeholder="이메일을 입력해주세요!" name = "userId" required autofocus><br>
        <button id="btn-Yes" class="btn btn-lg btn-primary btn-block" onclick = "check();">이메일 중복체크</button><br>
        <label for="inputPassword" class="sr-only">비밀번호</label>
        <input type="password" id="upwFirst" class="form-control" placeholder="비밀번호" name = "userPw" required><br>
        <label for="inputPassword" class="sr-only">비밀번호 확인</label>
        <input type="password" id="upwSecond" class="form-control" placeholder="비밀번호 확인" name = "userPw" required><br>
        <label for="" class="sr-only">이름</label>
        <input type="text" id="" class="form-control" placeholder="이름" name = "userName" required><br>
        <label for="" class="sr-only">닉네임</label>
        <input type="text" id="" class="form-control" placeholder="닉네임" name = "userNickName" required><br>
        <label for="" class="sr-only">전화번호</label>
        <select name="userTelnoOne" id="" class = "select">
            <option value="010" selected="selected">010</option>
        </select>
        <input type="text" name = "userTelnoTwo" minlength="4" class="control" required>
        <input type="text" name = "userTelnoThree" minlength="4" class="control" required><br>
        ...
        <input type="text" id="hint" class="form-control" placeholder="선택한 힌트를 입력해주세요!" name = "userHint" required><br>
        <button id="btn-Yes" class="btn btn-lg btn-primary btn-block" type="submit" name="" formaction = "UserInsert.do">회원가입</button>
    </form>
    <br>
