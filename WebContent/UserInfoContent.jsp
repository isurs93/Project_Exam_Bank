  ...
  <body cellpadding="0" cellspacing="0" marginleft="0" margintop="0" width="100%" height="100%">
    ...
    <div class="card align-middle" style="width:700px; border-radius:20px;">
      ...
      <form class="form-signin" method="POST" onSubmit="return pwCheck();">
        ...
        <input type="text" id="uid" class="form-control" name = "userSeq" value="<%=session.getAttribute("userSeq") %>" readonly="readonly" hidden="hidden"><br>        
        ...
        <input type="text" id="upwFirst" class="form-control" placeholder="비밀번호" name = "userPw" value="<%=session.getAttribute("userPw")%>" required><br>
        <label for="inputPassword" class="sr-only">비밀번호 확인</label>
        <input type="text" id="upwSecond" class="form-control" placeholder="비밀번호 확인" name = "userPw" value="<%=session.getAttribute("userPw")%>" required><br>
       ...
       <select name="userHintQuiz" id="" class = "hint" required>
        ...
        <input type="text" id="" class="form-control" placeholder="회원가입 일자" name = "userJoinDate" value="<%=session.getAttribute("userjoinDate") %>" readonly="readonly" required><br>
          <%if(session.getAttribute("useroutDate") == null) {%>
        <input type="text" id="" class="form-control" placeholder="회원탈퇴 일자" name = "outDate" value="현재 활동중인 회원입니다." required><br>
        <%}else{ %>
        <input type="text" id="" class="form-control" placeholder="회원탈퇴 일자" name = "outDate" value="<%=session.getAttribute("useroutDate") %>" required><br>
        <%} %>
        <%if((int)session.getAttribute("useradminSeq") != 1){ %>
        <input type="text" id="" class="form-control" placeholder="관리자 번호" value="일반회원 입니다."  required><br>
        <input type="text" id="" class="form-control" placeholder="관리자 번호" name = "adminSeq" value="0" required hidden="hidden"><br>
        <%}else{%>
        <input type="text" id="" class="form-control" placeholder="관리자 번호"  value="관리자 입니다." required><br>
        <input type="text" id="" class="form-control" placeholder="관리자 번호" name = "adminSeq" value="1" required hidden="hidden"><br>
        <%} %>
        <button id="btn-Yes" class="btn btn-lg btn-primary btn-block" type="submit" name="" formaction = "userManagementUpdate.do">회원정보 수정</button>
        <%if(session.getAttribute("useroutDate") == null) {%>
        <button id="btn-Yes" class="btn btn-lg btn-primary btn-block" type="submit" name="" formaction = "userManagementDrop.do">회원탈퇴</button>
        <%}else{ %>
        <button id="btn-Yes" class="btn btn-lg btn-primary btn-block" type="submit" name="" formaction = "userManagementDropCancle.do">회원복원</button>
        <%} %>
        <%if((int)session.getAttribute("useradminSeq") != 1){%>
        <button id="btn-Yes" class="btn btn-lg btn-primary btn-block" type="submit" name="" formaction = "userManagementAuthorization.do">관리자 권한 주기</button>
        <%}else{ %>
        <button id="btn-Yes" class="btn btn-lg btn-primary btn-block" type="submit" name="" formaction = "userManagementAuthorizationCancel.do">관리자 권한 취소</button>
		<%} %>        
    </form>
    
    	<br>
        <button id="btn-Yes" class="btn btn-lg btn-primary btn-block" name="" onclick = "location.href='userPurchasedList.do?userSeq=<%=session.getAttribute("userSeq") %>'" >구매 문제 보기</button>
    	<br>
