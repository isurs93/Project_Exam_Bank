...
%>
  <jsp:include page="IncludePage.jsp"/>
      ...
      <form action = "#">
        ...
        <table style = "align-items: center; position: relative; z-index: 1;">
        	<tr>
        		<td colspan = "5" style ="height:40px; width:50px; opacity: 95%;" >
            		...
            		<input type = "submit" name = "serachBtn" value = "검색" style="width:100px; height: 30px;  margin-right:50px;" formaction="list.do" >
          ...
          <tr>
             <td class = "no"><a href="contentView.do?postSeq=${dto.postSeq}">${dto.postSeq}</a></td>
             <td id = "title"> <a href="contentView.do?postSeq=${dto.postSeq}">${dto.title}</a></td>
			...
			<tr>
				...
				<%if(request.getParameter("index") != null){ // <<-,  <- 버튼 효시 할지 결정 %>
					<%if(Integer.parseInt(request.getParameter("index"))-5 < 0){%>
						<a href = "list.do?index=0"><i class="fas fa-angle-double-left "></i></a>
					<%}else{%>
						<a href = "list.do?index=<%=Integer.parseInt(request.getParameter("index"))-5%>"><i class="fas fa-angle-double-left "></i></a>
						
					<%}if(Integer.parseInt(request.getParameter("index"))-1 < 0){%>
						<a href = "list.do?index=0"><i class="fas fa-caret-left"></i></a>
					<%}else{%>
						<a href = "list.do?index=<%=Integer.parseInt(request.getParameter("index"))-1%>"><i class="fas fa-caret-left"></i></a>
					<%} %>
				<%}
				  else{%>
					  	<a href = "list.do?index=0"><i class="fas fa-angle-double-left "></i></a>
				...
				<%}
				  ...
				  if(session.getAttribute("ListCount") != null && request.getParameter("index") != null){
						...
						for(int index=StartNum; index<LastNum; index++){%>
							<a href="list.do?index=<%=index %>"><%=index+1 %></a>
					  <%}
						
						// 선택 되어진 페이지가 최대 페이지보다 작으면 -> (하나의 페이지이동) 버튼을 생성한다.
						if(pageNum < countNum/10+1){%>
							<%if(pageNum+1 < LastNum-1){%>
								 <a href = "list.do?index=<%=Integer.parseInt(request.getParameter("index"))+1%>"><i class="fas fa-caret-right"></i></a>
							<%}else{%>
								 <a href = "list.do?index=<%=LastNum-1%>"><i class="fas fa-caret-right"></i></a>
							<%}
							
							    // 그리고 선택 되어진 페이지에 5를 더했을때 최대 페이지를 넘어가면 
							    // ->> 버튼에 5 페이지 이동 기능을 빼준다.
							  if(pageNum+5 > countNum/10+1){ %>
									 <a href = "list.do?index=<%=LastNum-1%>" ><i class="fas fa-angle-double-right"></i></a>
						  	  <%}else{%>
						  		 	<a href = "list.do?index=<%=Integer.parseInt(request.getParameter("index"))+5%>" ><i class="fas fa-angle-double-right"></i></a>					  		 
					  		  <%}
						}						
				  }else{		
					// 리스트와 인덱스가 있을 경우, 즉 맨 처음 해당 리스트 페이지에 들어왔을 경 						
						...
						if(request.getParameter("select") != null || request.getParameter("query") != null)
							countNum = (double) dao.NoticeQueryCount(request.getParameter("select"), request.getParameter("query"));
						else
							countNum = (double) dao.NoticeCount();
					  ...
					  <%} // 그리고 >, >> 버튼도 만들어준다.%>
					 <%if(countNum > 10){%>
				     <a href = "list.do?index=1"><i class="fas fa-caret-right"></i></a>
				     <a href = "list.do?index=<%=LastNum%>"><i class="fas fa-angle-double-right"></i></a>
				     <%} %>
				  <%}%>
				</td>
			</tr>
		...
		if(session.getAttribute("userSeq") != null && (int)session.getAttribute("userSeq") == 1){ %>	
      	<table style = "align-items: center;">
      		<tr>
      			<td colspan = "5"><button onclick = "location.href='NoticeWrite.jsp'" style = "opacity:80%">공지사항 입력</button><td>
      		</tr>
      	</table>
      	<%}%>
      </div>
